...
The engine is `tools/github_no_git_sync.ps1`; configuration is `tools/github_sync_config.json`; log is `logs/github_sync.log`; local state is `.sync_state/local_manifest.json`. It scans the whole workspace recursively, includes code and all current/future project folders, does not use Git commands, reads credentials only from `GITHUB_TOKEN` or `GH_TOKEN`, and does not delete remote files unless `sync_deletions` is explicitly enabled.

`RUN_LIVE_EXCEL_SYNC.bat` is separate and is not the full-workspace synchronizer.

### GitHub Credential And Refresh Controls

The synchronizer accepts only `GITHUB_TOKEN` or `GH_TOKEN` from the process, Windows user, or machine environment. It validates access to `ahmedlabib33-boop/Samco-Project-Intelegent-Hub` and requires repository push permission before scanning remote content. A Codespaces user secret is available inside Codespaces only and does not authenticate the local Windows app.
...
```powershell
setx GITHUB_TOKEN "<new-repository-token>"
setx SYNC_ADMIN_PIN "<new-private-admin-pin>"
```
