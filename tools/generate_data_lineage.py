...
def item(page, section, displayed, source_file, columns, transform, function, scope="project", missing="Show zero, empty table, or selected-project setup state", validation="project_id must equal active project_id", source_folder="data/import_templates", output="Streamlit and selected-project exports"):
    return {
        "ui_page": page,
...
LINEAGE = [
    ...
    item("Overview", "Live alerts", "Priority correspondence threads", "letters_intelligence.xlsx", "Thread, Priority, Next Action", "Auto-ingest project inbox then show highest-priority threads", "dashboard.load_letters_workbook", source_folder="letters_intelligence"),
    ...
    item("Delay Analysis - TIA", "Uploads", "Recognized file inventory and include/exclude controls", "01-13 Delay TIA CSV files", "all source columns + project_id + source_file + source_folder + source_row", "Inspect selected project's Delay TIA folder only", "dashboard.build_steel_delay_template_inventory_df", source_folder="delay_analysis/steel_delay_tia_templates"),
    item("Delay Analysis - TIA", "Tables & Conclusion", "Canonical source tables and conclusion", "01-13 Delay TIA CSV files", "metadata, activity, supply, P6, relationships, clauses, IFC, payments, RFI, concurrency", "Build event/activity/supply/concurrency context without cross-project fallback", "dashboard.build_delay_tia_analysis_context", source_folder="delay_analysis/steel_delay_tia_templates"),
    item("Delay Analysis - TIA", "MEP Activities", "MEP activities, schedule, civil logic", "MEP Activities.csv + MEP Schedule.csv + MEP Civil Logic.csv", "activity IDs, names, durations, predecessors, successors, dates", "Read selected project bl folder and preserve civil logic", "dashboard.load_delay_tia_bl_sources", source_folder="bl"),
    item("Delay Analysis - TIA", "AI - TIA", "Readiness, fragnet, causation, concurrency, EOT", "02,03,04,05,06,07,08,09,10,11 CSVs", "event ID, activity ID, float, critical, longest path, overlap, evidence", "Hybrid retrospective TIA rules and selected-project source trace", "steel_delay_tia.run_steel_delay_tia_analysis", source_folder="delay_analysis/steel_delay_tia_templates"),
    ...
    item("Delay Analysis - TIA", "Download Reports", "DOCX/HTML/CSV/Primavera outputs", "canonical TIA datasets", "project_id, source_file, source_row, event_id, activity_id", "Generate into selected project reports path", "TIADirectorPackGenerator", source_folder="delay_analysis", output="projects/{project_folder_name}/reports"),
    ...
    item("Contract & Claims Intelligence Center", "Claims intelligence", "Documents, clauses, evidence mappings, drafts, rebuttals", "contract_claims.db + 2-contracts/source + 2-contracts/evidence", "project_id, document path/hash, clause, evidence, mapping score, draft", "Physically separate SQLite database per project; project trace columns added to UI frames", "contract_claims_center.* / dashboard claims tab", source_folder="2-contracts", output="projects/{project_folder_name}/exports/claims"),
