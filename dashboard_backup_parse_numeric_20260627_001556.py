...
def render_decision_command_bar(registry_df: pd.DataFrame, quality: dict[str, Any]) -> None:
    ...
    with col6:
        st.markdown(f"<div class='decision-filter-status'><span class='decision-filter-label'>Data quality</span><span class='decision-badge' style='background:{badge_bg};color:{badge_fg}'>{html.escape(badge_text)}</span></div>", unsafe_allow_html=True)
    with col7:
    ...
    with col8:
        if st.button("Reset filters", key="decision_reset_filters", use_container_width=True):
...
def render_status_badge(status: str) -> str:
    ...
    return f"<span class='decision-badge' style='background:{badge_bg};color:{badge_fg}'>{html.escape(badge_text)}</span>"
...
def render_decision_making_dashboard(projects_catalog_df: pd.DataFrame) -> None:
    ...
    st.markdown(
        ...
        .decision-badge{display:inline-block;border-radius:999px;padding:5px 9px;font-size:10px;font-weight:900;white-space:nowrap;max-width:118px;overflow:hidden;text-overflow:ellipsis}
                ...
                with card_cols[idx % 3]:
                    st.markdown(
                        ...
                        <div class='decision-card'>
                          ...
                          <p><b>Sector:</b> {html.escape(str(row['Sector']))}</p>
                          <p><b>Status:</b> <span class='decision-badge' style='background:{badge_bg};color:{badge_fg}'>{html.escape(badge_text)}</span></p>
