...
PRIME_LOGO_PATH = Path(r"C:\Users\pc\OneDrive\Desktop\01 Letters\labib tia folders\Logos\Prime logo.png")
SAMCO_ALT_LOGO_PATH = Path(r"C:\Users\pc\OneDrive\Desktop\01 Letters\labib tia folders\Logos\samco logo.png")
TIA_DIRECTOR_WORD_TEMPLATE_PATH = APP_DIR / "templates" / "time_impact_analysis_report_director_pack.docx"
GENERATED_OUTPUTS_DIR = APP_DIR / "generated_outputs"
CONTRACT_CLAIMS_DIRS = ccc.ensure_contract_claims_dirs(APP_DIR)
...
CONTRACT_CLAIMS_DB_PATH = APP_DIR / "construction_system.db"
...
def build_wbs_metrics():
    ...
    target_codes = [
        "The Big - P.01 - UP-20-April-26.CON.P02.B01",
        "The Big - P.01 - UP-20-April-26.CON.P02.B02",
        "The Big - P.01 - UP-20-April-26.CON.P01.B04",
        "The Big - P.01 - UP-20-April-26.CON.P01.B03",
    ]
    chart_rows = pd.DataFrame()
    if code_col:
        chart_rows = wbs_df[wbs_df[code_col].isin(target_codes)].copy()
...
<body>
  <main class="dash">
    ...
    <section class="bottom-strip">{bottom_html}<div class="brand">SAMCO EGYPT<small>Commitment | Quality | Delivery</small></div></section>
...
def _build_super_premium_report_frames(overview_metrics: dict, evm_metrics: dict, contract_metrics: dict, delay_metrics: dict, risk_metrics: dict, milestone_metrics: dict, activity_metrics: dict, s_curve_metrics: dict) -> dict[str, pd.DataFrame]:
    ...
    for sheet_name in ["From SAMCO to ACE", "From ACE to SAMCO"]:
        if sheet_name in letters and not letters[sheet_name].empty:
            df = letters[sheet_name].copy()
            if sheet_name == "From SAMCO to ACE":
                df["From"] = "SAMCO"
                df["To"] = "ACE-PM"
            else:
                df["From"] = "ACE-PM"
...
## Governance Rules
- Do not write inside Power BI import tables manually.
- Refresh source CSVs, then regenerate the workbook.
- Keep report control fields current for issue governance.
- Validate formulas and table names before publishing.
...
wbs_costs = project_data.get("wbs_costs", [])
letters = load_letters_workbook()
...
logo_html = f"<img src='data:image/png;base64,{logo_b64}' alt='SAMCO Egypt logo'>" if logo_b64 else ""
st.markdown(
    f"""
    <div class='samco-header'>
      <div class='samco-headline'>{logo_html}<div><div class='samco-title'>SAMCO Egypt - Projects Intelligence Hub</div><div class='samco-subtitle'>Integrated Project Controls System</div></div></div>
      <div class='samco-contract-info'>
        <div class='samco-info-item'><div class='samco-info-label'>Contractor</div><div class='samco-info-value'>SAMCO Egypt</div></div>
        <div class='samco-info-item'><div class='samco-info-label'>Employer</div><div class='samco-info-value'>Roya</div></div>
        ...
        <div class='samco-info-item'><div class='samco-info-label'>Project</div><div class='samco-info-value'>{overview_metrics.get('project_name') or project.get('project_name','Big Project')}</div></div>
      </div>
...
def build_steel_delay_template_inventory_df() -> pd.DataFrame:
    rows = []
    for path in sorted(STEEL_TIA_DIR.glob("*")):
...
def steel_tia_template_use_label(file_name: str) -> str:
    mapping = {
        ...
        "03- employer_steel_supply.csv": "Employer ROYA supply timing used for steel availability and stock-out calculation.",
        ...
        "10- samco_steel_supplied_at_site.csv": "SAMCO site steel visibility only; excluded from employer-steel delay calculation.",
...
def steel_tia_prepare_employer_supply_df(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Delivery Ref" in working.columns:
        working["Delivery Ref"] = working["Delivery Ref"].replace("", pd.NA).fillna(
            pd.Series([f"ROYA-SUPPLY-{idx + 1:03d}" for idx in range(len(working))], index=working.index)
        )
    if "Remarks" in working.columns:
        working["Remarks"] = working["Remarks"].replace("", pd.NA).fillna("Employer ROYA steel supply")
...
def build_delay_tia_context() -> dict:
    master_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "02- master_activity_steel_analysis.csv", STEEL_TIA_DIR / "01- master_activity_steel_analysis.csv"])
    employer_raw_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "03- employer_steel_supply.csv", STEEL_TIA_DIR / "02- employer_steel_supply.csv"])
    p6_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "04- p6_activity_export.csv", STEEL_TIA_DIR / "03- p6_activity_export.csv"])
    relationship_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "05- relationship_file.csv", STEEL_TIA_DIR / "04- relationship_file.csv"])
    contract_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "06- contract_library.csv", STEEL_TIA_DIR / "05- contract_library.csv"])
    ifc_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "07- ifc_conflict.csv", STEEL_TIA_DIR / "06- ifc_conflict.csv"])
    payments_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "08- payments.csv", STEEL_TIA_DIR / "07- payments.csv"])
    rfi_df = steel_tia_load_first_csv_or_empty([STEEL_TIA_DIR / "09- rfi_status.csv", STEEL_TIA_DIR / "08- rfi_status.csv"])
    bl_critical_path_df = steel_tia_load_csv_or_empty(STEEL_TIA_DIR / "09- bl_critical_path.csv")
    samco_df = steel_tia_load_first_csv_or_empty(
        [
            STEEL_TIA_DIR / "10- samco_steel_supplied_at_site.csv",
            STEEL_TIA_DIR / "09- samco_steel_supplied_at_site.csv",
        ]
    )
...
def load_delay_tia_local_support_files() -> dict[str, pd.DataFrame]:
    return {
        "rfi_detailed_df": steel_tia_load_first_csv_or_empty(
            [
                STEEL_TIA_DIR / "RFI Delay.csv",
                APP_DIR / "Delay tia improvement files" / "upload_ready_for_delay_tia" / "RFI Delay.csv",
            ]
        ),
...
def build_delay_tia_question_kpis(frames: dict[str, pd.DataFrame]) -> dict[str, float | int]:
    ...
    employer_df = delay_tia_frames_get_first(frames, ["steel_delay_tia_templates/03- employer_steel_supply.csv", "steel_delay_tia_templates/02- employer_steel_supply.csv"])
    samco_df = delay_tia_frames_get_first(frames, ["steel_delay_tia_templates/10- samco_steel_supplied_at_site.csv", "steel_delay_tia_templates/09- samco_steel_supplied_at_site.csv"])
...
def answer_delay_tia_question(question: str, frames: dict[str, pd.DataFrame]) -> tuple[str, list[tuple[str, pd.DataFrame]], dict[str, float | int]]:
    ...
    if any(word in q for word in ["final", "submit", "submission", "total", "days", "delay"]):
        ...
        answer = (
            ...
            "Employer ROYA steel is the calculation basis; SAMCO steel is shown only for visibility."
        )
        return answer, tables, kpis

    if any(word in q for word in ["rfi", "reply", "consultant"]):
        for key in ["Delay tia improvement files/18-rfi_delay_claim6_normalized.csv", "steel_delay_tia_templates/RFI Delay.csv", "Delay tia improvement files/upload_ready_for_delay_tia/RFI Delay.csv", "steel_delay_tia_templates/09- rfi_status.csv", "steel_delay_tia_templates/08- rfi_status.csv"]:
    ...
    if any(word in q for word in ["critical", "longest", "float", "activity", "path"]):
        for key in ["Delay tia improvement files/03-activity_impact_register_template.csv", "steel_delay_tia_templates/04- p6_activity_export.csv", "steel_delay_tia_templates/03- p6_activity_export.csv"]:
            df = frames.get(key, pd.DataFrame())
            if not df.empty:
...
def build_delay_tia_director_readiness_matrix_df(context: dict, analysis: dict) -> pd.DataFrame:
    rows = [
        ...
        ("SAMCO site supply visibility", "Low", "Display-only stream for management visibility, excluded from TIA calculations.", "Complete" if not context.get("samco_df", pd.DataFrame()).empty else "Not Required"),
    ]
...
def build_delay_tia_director_metadata(context: dict, analysis: dict) -> dict:
    return {
        "project_name": "Project Intelligence Hub - Delay TIA",
        "contract_no": "NOT PROVIDED IN UPLOADED FILES",
        "employer": "ROYA / Employer",
        "contractor": "SAMCO / Contractor",
        "contract_form_or_clause": "From uploaded contract library and matched clauses",
        "accepted_baseline_programme": "BL fixed package plus uploaded activity export",
...
def build_delay_tia_detailed_report_html(context: dict, analysis: dict) -> str:
    ...
    methodology_df = pd.DataFrame(
        [
            {"Step": 1, "Methodology Logic": "Use uploaded employer steel supply only; exclude SAMCO supply from calculations."},
...
def render_contract_clause_matching_engine():
    ...
    event_description = col2.text_input("Event Description", value="Steel delivery delayed by 46 days")
...
with tabs[7]:
    ...
    if not letters:
        ...
        samco_links = letters.get("SAMCO to ACE Links", pd.DataFrame())
        ace_links = letters.get("ACE to SAMCO Links", pd.DataFrame())
        threads = letters.get("Issue Threads", pd.DataFrame())
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("SAMCO Letters", len(samco), delta="Outbound")
        c2.metric("ACE Letters", len(ace), delta="Inbound")
        c3.metric("Linked SAMCO Threads", len(samco_links), delta="Relationship map")
        ...
        with col1:
            if not samco.empty:
                ...
                st.plotly_chart(style_plotly(px.bar(counts, x="Risk Type", y="Count", title="SAMCO Risk Subjects", color="Risk Type", color_discrete_sequence=px.colors.qualitative.Safe)), use_container_width=True)
...
with tabs[11]:
    ...
    with tia_tabs[0]:
        ...
        st.markdown(
            ...
            "Employer ROYA steel is the only steel supply used in all delay-analysis calculations. "
            "Contractor-SAMCO steel is displayed for visibility only and is excluded from stock-out, affected-activity, "
        ...
        delay_tia_file_specs = [
            ...
            {
                "key": "employer",
                "label": "03- employer_steel_supply.csv",
                "required": True,
        ...
        delay_tia_improvement_file_specs = [
            ...
            },
            {
                ...
                "fields_used": "Primary Event ID, Activity ID, Activity Name, BL Start, BL Finish, Start, Finish, Delayed duration, Overlap Start, Overlap Finish, BL Critical Path, Current Critical Path, Delayed duration after overlap, Concurrent delay",
                "tia_use": "Optional concurrency support for final event treatment using overlap windows, BL/current criticality, and concurrent-delay days.",
            },
            {
        ...
        include_samco_delay = include_col4.checkbox("Include SAMCO supposed delay", value=True, key="delay_tia_include_samco")
        ...
        included_delay_streams_df = pd.DataFrame(
            [
                ...
                {"Delay Stream": "SAMCO site steel visibility", "Included": "Yes" if include_samco_delay else "No"},
            ]
        )
        ...
        st.caption("BL comparison sources are loaded automatically from the local `BL fixed` folder. No upload is required for baseline comparison.")
    ...
    with tia_tabs[1]:
        ...
        if not delay_tia_ready:
            ...
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Employer ROYA Supplied Qty", f"{active_delay_tia_context['employer_total']:,.3f}")
            c2.metric("Contractor-SAMCO Supplied Qty", f"{active_delay_tia_context['samco_total']:,.3f}")
    ...
    with tia_tabs[1]:
        ...
        steel_logic_df = pd.DataFrame(
            [
                {"Rule": "Supply source", "Program logic": "Only Employer ROYA steel is used in calculations. SAMCO site steel is display-only."},
    ...
    with tia_tabs[1]:
        ...
        priority_df = pd.DataFrame(
            [
                {"Priority": 1, "File / Source": "02- master_activity_steel_analysis.csv", "Why it matters": "Primary activity-level shortage and requirement proof"},
                {"Priority": 2, "File / Source": "03- employer_steel_supply.csv", "Why it matters": "Employer ROYA supply timing used in all steel delay calculations"},
                {"Priority": 3, "File / Source": "04- p6_activity_export.csv", "Why it matters": "Readiness, float, criticality, and schedule context"},
                {"Priority": 4, "File / Source": "05- relationship_file.csv", "Why it matters": "Sequence impact and fragnet positioning"},
                {"Priority": 5, "File / Source": "06- contract_library.csv", "Why it matters": "Contract support, notice, time bar, money and schedule impact"},
                {"Priority": 6, "File / Source": "07- ifc_conflict.csv / 08- payments.csv / 09- rfi_status.csv", "Why it matters": "Support delay analyzers for non-steel event families"},
        ...
        st.markdown(
            ...
            - `03- employer_steel_supply.csv` stays a **delivery event file**
    ...
    with tia_tabs[3]:
        if not include_payments_delay:
            ...
            if not payments_df.empty:
                paid_total = numeric_sum_from_columns(payments_df, ["paid_amount", "paid amount", "paid"])
                certified_total = numeric_sum_from_columns(payments_df, ["certified_amount", "certified amount", "certified"])
                c1, c2, c3 = st.columns(3)
    ...
    with tia_tabs[13]:
        ...
        st.caption("This slide combines steel delay days from the master activity file, detailed RFI delay days from `RFI Delay.csv`, and the IFC revision-cycle delay from `07- ifc_conflict.csv`, then links each row to current float, criticality, and longest-path status.")
        if not delay_tia_ready:
    ...
    if not output_unlocked:
        ...
        if output_mode == "The Big - Dashboard":
            ...
            st.download_button(
                ...
                file_name="The Big - Dashboard.html",
                mime="text/html",
                use_container_width=True,
            )
        elif output_mode == "Original presentation print-only":
            st.markdown("<div class='panel-note'><b>Original Presentation</b><br>This path preserves the original presentation design and colors, but updates the linked slide content from the live platform data each time you export.</div>", unsafe_allow_html=True)
            ...
            c1.download_button(
                ...
                file_name="08-Presentations the big - 11.5.2026-updated.pptx",
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True,
            ...
            c2.download_button(
                ...
                data=original_template_bytes,
                file_name="08-Presentations the big - 11.5.2026-updated-print.pptx",
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True,
                disabled=not bool(original_template_bytes),
            )
            st.caption("This option keeps the original presentation design fixed and updates the linked content from the platform data at export time.")
            ...
            linked_export_col1.download_button(
                ...
                file_name="linked_executive_dashboard.html",
                mime="text/html",
                use_container_width=True,
            )
            linked_export_col2.download_button(
                ...
                file_name="linked_executive_dashboard_landscape.pptx",
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True,
            )
            st.caption("The PowerPoint export is generated in widescreen landscape format for cleaner executive printing.")
            ...
            linked_export_col_summary_html.download_button(
                ...
                file_name="linked_executive_dashboard_a3_landscape_one_page.html",
                mime="text/html",
                use_container_width=True,
            )
            linked_export_col_summary.download_button(
                ...
                file_name="linked_executive_dashboard_a3_landscape_one_page.pptx",
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True,
            )
            linked_export_col3.download_button(
                ...
                data=evm_html.encode("utf-8"),
                file_name="linked_executive_dashboard_evm_analysis.html",
                mime="text/html",
                use_container_width=True,
            )
            linked_export_col4.download_button(
                ...
                data=evm_ppt,
                file_name="linked_executive_dashboard_evm_analysis.pptx",
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True,
            )
...
with tabs[14]:
    ...
    st.markdown(
        "<div class='panel-note'><b>Live Review Room</b><br>Use this slide to join a live audio/video call while reviewing the Project Intelligence Hub together. The room is embedded inside the dashboard so you can talk through each slide without leaving the app.</div>",
        unsafe_allow_html=True,
    )

    default_room_seed = overview_metrics.get("project_name") or "Project-Intelligence-Hub"
    default_room_name = sanitize_conference_room_name(f"{default_room_seed}-Review")
    call_col1, call_col2, call_col3 = st.columns(3)
    conference_room_name = call_col1.text_input(
        "Room Name",
        value=st.session_state.get("conference_room_name", default_room_name),
    ...
    conference_subject = mode_col3.text_input(
        "Meeting Subject",
    ...
    st.markdown(f"Join link: [{room_url}]({room_url})")

    with st.expander("Participants and instructions", expanded=False):
        st.write("Share the room link with any participant. They can join from browser without opening a separate project application.")
        st.write("If you only want voice, keep audio-first mode enabled and video muted.")
