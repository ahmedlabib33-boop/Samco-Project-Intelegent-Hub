...
$repoUrl = "https://github.com/ahmedlabib33-boop/Samco-Project-Intelegent-Hub.git"
$syncRoot = Join-Path $env:TEMP ("samco-project-hub-live-sync-worktree-" + [guid]::NewGuid().ToString("N"))

Push-Location $root
try {
    $remoteNames = & $git remote
    if ($remoteNames -notcontains "samco") {
        & $git remote add samco $repoUrl
    }
    & $git fetch samco main
    ...
    & $git worktree prune
    & $git worktree add --detach $syncRoot samco/main
} finally {
    Pop-Location
}
